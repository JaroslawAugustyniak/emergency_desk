...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log(`${API_URL}/api/locations?${params.toString()}`);
    const res = await fetch(`${API_URL}/api/locations?${params.toString()}`, {
      method: 'GET',
      headers: {
    ...
    console.log('GET /api/locations response:', {
      status: res.status,
      contentType,
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('POST /api/locations response:', {
      status: res.status,
      contentType,
      bodyLength: text.length,
      bodyPreview: text.substring(0, 500),
      ...
      return NextResponse.json(
        {
          ...
          message: data.message || 'Failed to create location',
        },
        { status: res.status }
      );
    }
