...
export async function PATCH(
  ...
  try {
    ...
    const res = await fetch(`${API_URL}/api/orders/${orderId}/pause`, {
      method: 'PATCH',
      headers: {
    ...
    console.log(`PATCH /api/orders/${orderId}/pause response:`, {
      status: res.status,
      contentType,
    ...
    if (!res.ok) {
      ...
      return NextResponse.json(
        {
          error: data.message || 'Failed to pause order',
          details: data.errors || data.detail || null,
          message: data.message || 'Failed to pause order',
        },
        { status: res.status }
      );
    }
