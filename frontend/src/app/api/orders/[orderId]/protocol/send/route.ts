...
export async function POST(
  ...
  try {
    ...
    const res = await fetch(`${API_URL}/api/orders/${orderId}/protocol/send`, {
      method: 'POST',
      headers: {
    ...
    console.log(`POST /api/orders/${orderId}/protocol/send response:`, {
      status: res.status,
      contentType,
    ...
    if (!res.ok) {
      ...
      return NextResponse.json(
        {
          error: data.message || 'Failed to send protocol',
          message: data.message || 'Failed to send protocol',
        },
        { status: res.status }
      );
    }
