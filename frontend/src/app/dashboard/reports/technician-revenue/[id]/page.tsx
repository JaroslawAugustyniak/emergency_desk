...
export default function TechnicianDetailPage() {
  ...
  const handleDateRangeChange = useCallback((from: string, to: string) => {
    ...
    router.push(`/dashboard/reports/technician-revenue/${technicianId}?${params.toString()}`);
  }, [router, technicianId]);
    ...
    const timer = setTimeout(() => {
      const fetchData = async () => {
        try {
          ...
          const result = await getTechnicianRevenueDetail(technicianId, fromDate, toDate, token);
          setData(result);
        } catch (err) {
  ...
  }, [fromDate, toDate, token, technicianId]);
